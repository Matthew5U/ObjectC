			...
			<div class="ui two column stackable relaxed grid">
  				...
  				<div class="eleven wide column">
					...
					if ( isset($_POST['add']) ) {
						...
						// Reprezentacja zapytania do bazy
						$statement = $mysqli->prepare("INSERT article (title,image,content) VALUES (?,?,?)"); // Pytajniki bo do 3 kolumn
						$statement->bind_param("sss", $title, $image, $content);// Wstawiamy parametry nam iejsce pytajnikow (?,?,?) // sss string string string
						//Wykonanie zapytania
						$statement->execute();
						$statement->close(); // Zamkniecie zapytania
						header("Location: index.php"); // Przeladowanie strony aby nie wstawiac tych smaych wartosci jeszcze raz
					}
					?>
